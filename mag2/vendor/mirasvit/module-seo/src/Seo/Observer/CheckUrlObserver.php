...
class CheckUrlObserver implements ObserverInterface
{
    ...
    protected function redirectFromRedirectManagerUrlList($response)
    {
        ...
        $redirectCollection->getSelect()
            ->where('url_from = ' . "'" . $currentUrl . "'"
                . ' OR ' . 'url_from = ' . "'" . $url . "'"
                . ' OR ' . 'url_from = ' . "'" . $trimmedUrl . "'"
                . ' OR ' . "'" . $url . "'" . " LIKE CONCAT(REPLACE(url_from, '*', '%'))"
                . ' OR ' . "'" . $trimmedUrl . "'" . " LIKE CONCAT(REPLACE(url_from, '*', '%'))"
                . ' OR ' . "'" . $currentUrl . "'" . " LIKE CONCAT(REPLACE(url_from, '*', '%'))")
        ...
        foreach ($redirectCollection as $redirect) {
            ...
            if ($this->redirectHelper->checkRedirectPattern($redirect->getUrlFrom(), $redirect->getUrlTo())) {
                continue;
            }
