...
class FilterSetting extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    {
        $filterCode = self::ATTR_PREFIX . $attributeModel->getAttributeCode();
        $setting = $this->collection->getItemByColumnValue(Shopby\Model\FilterSetting::FILTER_CODE, $filterCode);
        if (is_null($setting)) {
    ...
    {
        if($layerFilter instanceof \Magento\CatalogSearch\Model\Layer\Filter\Category) {
            return self::ATTR_PREFIX . 'category_ids';
        }
        try
        {
