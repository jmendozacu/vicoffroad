...
class Copyrelate extends \Amasty\Paction\Model\Command
{
    ...
    {
        $db     = $this->connection;
        $table  = $this->connection->getTableName('catalog_product_link');
    ...
    protected function _getLinkAttributeId($code = 'position')
    {
        if (is_null($this->_link_attribute_id)) {
            $db    = $this->connection;
            $table = $this->connection->getTableName('catalog_product_link_attribute');
    ...
    protected function _createNewLink($productId, $linkedProductId, $parentLinkId)
    {
        ...
        if (!$row) {
            ...
            $table = $this->connection->getTableName('catalog_product_link_attribute_' . $this->_link_attribute_data_type);
            $select = $db->select()->from($table)
