    ...
    public function execute($ids, $storeId, $val)
    {
        ...
        foreach ($codes as $code) {
            ...
            if (!$attribute || !$attribute->getId()) {
                throw new \Amasty\Paction\Model\CustomException(__('There is no product attribute with code `%s`, please compare values in the module configuration with stores > attributes > product.', $code));
            }
            
            if ($attribute->getIsUnique()) {
                throw new \Amasty\Paction\Model\CustomException(__('Attribute `%s` is unique and can not be copied. Please remove the code in the module configuration.', $code));
            }
            
            $type = $attribute->getBackendType();
            if ('static' == $type) {
                throw new \Amasty\Paction\Model\CustomException(__('Attribute `%s` is static and can not be copied. Please remove the code in the module configuration.', $code));
            }
    ...
    protected function _copyData($fromId, $ids, $config)
    {
       $db = $this->connection;
        foreach ($config as $type => $attributes) {
            ...
            $table  = $this->connection->getTableName('catalog_product_entity_' . $type);
            foreach ($ids as $id) {
                $sql = "INSERT INTO $table (`attribute_id`, `store_id`, `entity_id`, `value`) "
                     . " SELECT  t.`attribute_id`, t.`store_id`, $id, t.`value`"
                     . " FROM $table AS t "
                     . " WHERE t.`entity_id`=$fromId AND t.`attribute_id` IN($attributes)"
