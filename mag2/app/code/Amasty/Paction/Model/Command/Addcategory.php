...
class Addcategory extends \Amasty\Paction\Model\Command
{
    ...
    public function execute(array $ids, $storeId, $categoryIds)
    {
        ...
        if ('replacecategory' == $this->_type) { // remove product(s) from all categories
            ...
            $this->connection->delete($table, array('product_id IN(?)' => $ids));
            $this->_type = 'addcategory';
        }
        ...
        foreach ($categoryIds as $categoryId) {
            ...
            try {
                ...
                $this->_errors[] = __('Can not handle the category ID=%1, the error is: %2',
                    $categoryId, $e->getMessage());
            }     
        }
        $success='';
        if ($numAffectedCats) {
            $success = __('Total of %1 categories(s) and %1 products(s) have been successfully updated.',
