...
class Copyimg extends \Amasty\Paction\Model\Command
{
    ...
    protected function _copyData($attrId, $originalProductId, $newProductId, $product, $imageAttr)
    {
        ...
        $tableValueToEntity = $this->connection->getTableName('catalog_product_entity_media_gallery_value_to_entity');
        ...
        $select = $db->select()
            ->from($table, array('value_id', 'value'))->joinInner(
                ['entity' => $this->connection->getTableName(\Magento\Catalog\Model\ResourceModel\Product\Gallery::GALLERY_VALUE_TO_ENTITY_TABLE)],
