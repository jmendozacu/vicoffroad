...
class Modifyprice extends \Amasty\Paction\Model\Command
{
    ...
    public function execute($ids, $storeId, $val)
    {
        if (!preg_match('/^[+-][0-9]+(\.[0-9]+)?%?$/', $val)) {
            throw new \Magento\Framework\Exception\LocalizedException(__('Please provide the difference as +12.5, -12.5, +12.5% or -12.5%'));
        }
        ...
        $val = floatval($val);
        if ($val < 0.00001) {
            throw new \Magento\Framework\Exception\LocalizedException(__('Please provide a non empty difference'));
        }
